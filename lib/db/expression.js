...
const UPDATES = {
  // Set
  SET: (operand) => new Operator('SET', (a, b) => `${a} = ${b}`, [operand]),
  ADD: (operand) => new Operator('SET', (a, b) => `${a} = ${a} + ${b}`, [operand]),
  SUB: (operand) => new Operator('SET', (a, b) => `${a} = ${a} - ${b}`, [operand]),
  LIST_SET: (index, operand) => new Operator('SET', (a, b, c) => `${a}[${b}] = ${c}`, [index, operand]),
  LIST_PUSH: (operand) => new Operator('SET', (a, b) => `${a} = list_append(${a}, ${b})`, [operand]),
  IF_NOT_EXISTS: (operand) => new Operator('SET', (a, b) => `${a} = if_not_exists(${a}, ${b})`, [operand]),
  // Remove
  REMOVE: () => new Operator('REMOVE', (a) => `${a}`, []),
  LIST_REMOVE: (...index) => new Operator('REMOVE', (a, ...b) => b.map(bb => `${a}[${bb}]`).join(', '), index),
  // Delete
  DELETE: (operand) => new Operator('DELETE', (a, b) => `${a} ${b}`, [operand])
}

const CONDITIONS = {
  // - Logic
  AND: (...conditions) => new Operator('AND', (...a) => a.map(aa => `(${aa})`).join(' AND '), conditions),
  OR: (...conditions) => new Operator('OR', (...a) => a.map(aa => `(${aa})`).join(' OR '), conditions),
  NOT: (exp) => new Operator('NOT', (a) => `NOT (${a})`, [exp]),
  // - Comparisions
  EQ: (val) => new Operator('EQ', (a, b) => `${a} = ${b}`, [val]),
  GT: (val) => new Operator('GT', (a, b) => `${a} > ${b}`, [val]),
  LT: (val) => new Operator('LT', (a, b) => `${a} < ${b}`, [val]),
  GTE: (val) => new Operator('GTE', (a, b) => `${a} >= ${b}`, [val]),
  LTE: (val) => new Operator('LTE', (a, b) => `${a} <= ${b}`, [val]),
  IN: (...vals) => new Operator('IN', (a, ...b) => `${a} IN (${b.join(', ')})`, vals),
  BTW: (min, max) => new Operator('BTW', (a, b, c) => `${a} BETWEEN ${b} AND ${c}`, [min, max])
  // - Functions
  // SIZE: (path) => new Operator('SIZE', (a) => `size(${a})`, path),
  // EXISTS: (path) => new Operator('EXISTS', (a) => `attribute_exists(${a})`, path),
  // NOT_EXISTS: (path) => new Operator('NOT_EXISTS', (a) => `attribute_not_exists(${a})`, path),
  // CONTAINS: (path, operand) => new Operator('CONTAINS', (a, b) => `contains(${a}, ${b})`, [path, operand]),
  // IS_TYPE: (path, type) => new Operator('IS_TYPE', (a, b) => `attribute_type(${a}, ${b})`, [path, type]),
  // BEGINS_WITH: (path, substr) => new Operator('BEGINS_WITH', (a, b) => `begins_with(${a}, ${b})`, [path, substr])
}
  ...
  const iter = (obj, path = []) => {
    ...
    for (const [key, val] of Object.entries(obj)) {
      switch (val.constructor.name) {
        ...
        case 'Object': {
          names[`#${key}`] = key
          const { actions: _actions, names: _names, values: _values } = iter(val, [...path, key])
  ...
  const iter = (obj, path = []) => {
    ...
    if (key.startsWith('$') && CONDITIONS[key.slice(1).toUpperCase()]) {
      ...
      switch (key.slice(1).toUpperCase()) {
        case 'AND': {
          if (!Array.isArray(value)) throw new Error('invalid value type, should be Array')
