...
const { DB: { Region, Endpoint, TableName, DefaultLimit }, isDevelopment, isTest } = require('../../config')

// Default client
const dbOptions = isTest ? {
  region: 'local-env',
...
const Client = new DocumentClient(dbOptions)
 ...
 */
const Update = async (key, update, { client = Client, returnValue = 'UPDATED_NEW' } = DefaultOptions) => {
